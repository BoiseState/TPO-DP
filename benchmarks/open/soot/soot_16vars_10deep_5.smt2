;symbolic execution tree
(push)
(assert (le x0 java.lang.Object))
(check-sat)
(push)
(assert (le x1 java.lang.Object))
(check-sat)
(push)
(assert (le x2 java.lang.Object))
(check-sat)
(push)
(assert (le x3 java.lang.Object))
(check-sat)
(push)
(assert (le x4 java.lang.Object))
(check-sat)
(push)
(assert (le x5 java.lang.Object))
(check-sat)
(push)
(assert (le x6 java.lang.Object))
(check-sat)
(push)
(assert (le x7 java.lang.Object))
(check-sat)
(push)
(assert (le x8 java.lang.Object))
(check-sat)
(push)
(assert (le x9 java.lang.Object))
(check-sat)
(push)
(assert (le x10 java.lang.Object))
(check-sat)
(push)
(assert (le x11 java.lang.Object))
(check-sat)
(push)
(assert (le x12 java.lang.Object))
(check-sat)
(push)
(assert (le x13 java.lang.Object))
(check-sat)
(push)
(assert (le x14 java.lang.Object))
(check-sat)
(push)
(assert (le x15 java.lang.Object))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x13 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x6 java.lang.StringBuffer)))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.StringBuffer)))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x2 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x13 java.lang.StringBuilder))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x15 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x15 java.util.Map_currencysign_Entry))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.util.Map_currencysign_Entry)))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x7 java.io.File))
(check-sat)
(push)
(assert (le x1 java.io.DataInputStream))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.io.DataInputStream)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x13 java.lang.StringBuilder)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x13 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x10 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x4 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x4 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x13 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(push)
(assert (= x1 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x10 java.io.DataInputStream))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.io.DataInputStream)))
(check-sat)
(pop)
(push)
(assert (le x7 soot.coffi.CONSTANT_Class_info))
(check-sat)
(pop)
(push)
(assert (not (le x7 soot.coffi.CONSTANT_Class_info)))
(check-sat)
(push)
(assert (le x6 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.io.File)))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x1 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (not (le x7 soot.coffi.CONSTANT_Fieldref_info)))
(check-sat)
(pop)
(push)
(assert (le x7 soot.coffi.CONSTANT_Fieldref_info))
(check-sat)
(pop)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(push)
(assert (le x10 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x4 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(pop)
(push)
(assert (not (le x4 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(pop)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x14 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x10 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x1 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x1 soot.coffi.Code_attribute))
(check-sat)
(pop)
(push)
(assert (not (le x1 soot.coffi.Code_attribute)))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (le x15 java.util.HashMap))
(check-sat)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x15 java.util.HashMap)))
(check-sat)
(pop)
(push)
(assert (le x6 soot.coffi.CONSTANT_Methodref_info))
(check-sat)
(pop)
(push)
(assert (not (le x6 soot.coffi.CONSTANT_Methodref_info)))
(check-sat)
(pop)
(push)
(assert (le x8 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x11 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.io.File)))
(check-sat)
(push)
(assert (le x2 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(push)
(assert (le x11 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x2 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x8 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x6 soot.coffi.CONSTANT_Methodref_info))
(check-sat)
(pop)
(push)
(assert (not (le x6 soot.coffi.CONSTANT_Methodref_info)))
(check-sat)
(pop)
(push)
(assert (le x8 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(push)
(assert (le x9 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(push)
(assert (le x9 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x11 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(pop)
(push)
(assert (not (le x11 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(pop)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x9 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x10 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x10 java.lang.StringBuffer)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(push)
(assert (= x1 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.StringBuffer)))
(check-sat)
(push)
(assert (le x7 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x6 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(push)
(assert (le x7 java.io.DataInputStream))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.io.DataInputStream)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x6 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x12 soot.tagkit.Tag))
(check-sat)
(pop)
(push)
(assert (not (le x12 soot.tagkit.Tag)))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(push)
(assert (le x7 java.io.File))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(push)
(assert (le x11 java.io.DataInputStream))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.io.DataInputStream)))
(check-sat)
(pop)
(push)
(assert (le x7 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (le x8 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x3 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.io.File)))
(check-sat)
(push)
(assert (le x7 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x7 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x8 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.io.File)))
(check-sat)
(push)
(assert (= x12 java.lang.StringBuilder))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.StringBuilder)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (le x13 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x6 soot.coffi.method_info))
(check-sat)
(pop)
(push)
(assert (not (le x6 soot.coffi.method_info)))
(check-sat)
(pop)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x2 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x7 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x7 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(push)
(assert (le x13 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(push)
(assert (le x8 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(push)
(assert (le x2 java.io.File))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x2 soot.SootClass))
(check-sat)
(pop)
(push)
(assert (not (le x2 soot.SootClass)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(push)
(assert (le x10 java.util.List))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.util.List)))
(check-sat)
(pop)
(push)
(assert (le x1 java.io.DataInputStream))
(check-sat)
(push)
(assert (le x4 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x4 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x1 java.io.DataInputStream)))
(check-sat)
(pop)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x15 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x15 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x15 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x7 soot.options.JBOptions))
(check-sat)
(pop)
(push)
(assert (not (le x7 soot.options.JBOptions)))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(push)
(assert (le x15 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x7 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x12 java.lang.StringBuilder))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.StringBuilder)))
(check-sat)
(pop)
(push)
(assert (le x6 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(push)
(assert (le x8 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x6 soot.Type))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x6 soot.Type)))
(check-sat)
(push)
(assert (le x13 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.io.File)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(push)
(assert (le x1 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x6 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(push)
(assert (le x1 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.io.File)))
(check-sat)
(push)
(assert (le x13 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x4 java.lang.Object))
(check-sat)
(pop)
(push)
(assert (not (le x4 java.lang.Object)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(push)
(assert (le x6 java.util.List))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.util.List)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(push)
(assert (le x10 soot.toolkits.scalar.BackwardFlowAnalysis))
(check-sat)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(push)
(assert (= x4 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x15 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(push)
(assert (le x11 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.io.File)))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (le x15 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x10 soot.toolkits.scalar.BackwardFlowAnalysis)))
(check-sat)
(push)
(assert (le x3 java.io.File))
(check-sat)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x3 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x7 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x7 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(pop)
(push)
(assert (not (le x7 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(push)
(assert (le x10 soot.toolkits.scalar.SimpleLiveLocalsAnalysis))
(check-sat)
(pop)
(push)
(assert (not (le x10 soot.toolkits.scalar.SimpleLiveLocalsAnalysis)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (le x10 soot.toolkits.scalar.FlowAnalysis))
(check-sat)
(pop)
(push)
(assert (not (le x10 soot.toolkits.scalar.FlowAnalysis)))
(check-sat)
(pop)
(push)
(assert (= x1 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x4 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x4 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x7 soot.coffi.CONSTANT_Methodref_info))
(check-sat)
(pop)
(push)
(assert (not (le x7 soot.coffi.CONSTANT_Methodref_info)))
(check-sat)
(pop)
(push)
(assert (le x7 soot.Type))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(push)
(assert (le x10 soot.toolkits.scalar.FlowAnalysis))
(check-sat)
(push)
(assert (le x12 java.lang.Object))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.lang.Object)))
(check-sat)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(push)
(assert (not (le x12 soot.RefType)))
(check-sat)
(pop)
(push)
(assert (le x12 soot.RefType))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (le x10 soot.toolkits.scalar.FlowAnalysis)))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(push)
(assert (le x10 soot.toolkits.scalar.BackwardFlowAnalysis))
(check-sat)
(pop)
(push)
(assert (not (le x10 soot.toolkits.scalar.BackwardFlowAnalysis)))
(check-sat)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(push)
(assert (le x12 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x7 soot.Type)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(push)
(assert (le x12 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x14 java.io.File))
(check-sat)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(push)
(assert (le x15 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.io.File)))
(check-sat)
(push)
(assert (= x11 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.StringBuffer)))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(push)
(assert (le x14 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x7 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(push)
(assert (le x1 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.io.File)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(push)
(assert (= x10 java.lang.StringBuilder))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.StringBuilder)))
(check-sat)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x14 java.io.File)))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x4 java.io.File))
(check-sat)
(push)
(assert (le x13 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(pop)
(push)
(assert (not (le x13 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x4 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x4 soot.coffi.Code_attribute))
(check-sat)
(push)
(assert (not (le x4 soot.coffi.Exception_attribute)))
(check-sat)
(pop)
(push)
(assert (le x4 soot.coffi.Exception_attribute))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x4 soot.coffi.Code_attribute)))
(check-sat)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(push)
(assert (le x1 soot.coffi.Code_attribute))
(check-sat)
(pop)
(push)
(assert (not (le x1 soot.coffi.Code_attribute)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(push)
(assert (le x1 java.io.File))
(check-sat)
(push)
(assert (le x10 soot.coffi.Exception_attribute))
(check-sat)
(pop)
(push)
(assert (not (le x10 soot.coffi.Exception_attribute)))
(check-sat)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x1 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(push)
(assert (le x15 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x7 soot.coffi.method_info))
(check-sat)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 soot.coffi.method_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(push)
(assert (le x8 soot.coffi.CONSTANT_NameAndType_info))
(check-sat)
(pop)
(push)
(assert (not (le x8 soot.coffi.CONSTANT_NameAndType_info)))
(check-sat)
(push)
(assert (le x6 java.io.File))
(check-sat)
(push)
(assert (le x12 java.io.File))
(check-sat)
(push)
(assert (= x15 java.lang.StringBuilder))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.StringBuilder)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x6 java.io.File)))
(check-sat)
(push)
(assert (= x11 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.StringBuffer)))
(check-sat)
(push)
(assert (le x6 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x15 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x11 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x14 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (le x11 java.io.File))
(check-sat)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x11 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x14 java.util.List))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.util.List)))
(check-sat)
(push)
(assert (le x10 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(push)
(assert (= x12 java.lang.StringBuilder))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x10 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x8 java.io.DataInputStream))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.io.DataInputStream)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x12 java.lang.StringBuilder)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x11 soot.util.HashChain_currencysign_Link))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x11 soot.util.HashChain_currencysign_Link)))
(check-sat)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(push)
(assert (le x12 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(push)
(assert (= x13 java.lang.StringBuilder))
(check-sat)
(push)
(assert (= x13 java.lang.StringBuilder))
(check-sat)
(push)
(assert (= x3 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x13 java.lang.StringBuilder)))
(check-sat)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x11 java.lang.Object))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x11 java.lang.Object)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x13 java.lang.StringBuilder)))
(check-sat)
(push)
(assert (= x2 java.lang.StringBuilder))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.StringBuilder)))
(check-sat)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(push)
(assert (le x7 soot.toolkits.scalar.FlowSet))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 soot.toolkits.scalar.FlowSet)))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x11 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x11 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x11 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x11 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x11 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(push)
(assert (= x2 java.lang.StringBuilder))
(check-sat)
(push)
(assert (le x9 soot.toolkits.scalar.ArraySparseSet))
(check-sat)
(pop)
(push)
(assert (le x9 soot.toolkits.scalar.AbstractFlowSet))
(check-sat)
(push)
(assert (not (le x9 soot.toolkits.scalar.ArraySparseSet)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x9 soot.jimple.toolkits.annotation.arraycheck.BoundedPriorityList))
(check-sat)
(pop)
(push)
(assert (le x9 soot.jimple.spark.ondemand.WrappedPointsToSet))
(check-sat)
(pop)
(push)
(assert (le x9 java.util.Hashtable))
(check-sat)
(pop)
(push)
(assert (le x9 polyglot.util.TypedList))
(check-sat)
(pop)
(push)
(assert (le x9 soot.jimple.toolkits.pointer.MemoryEfficientRasUnion))
(check-sat)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.StringBuilder))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.StringBuilder)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x9 soot.jimple.spark.ondemand.genericutil.ImmutableStack))
(check-sat)
(pop)
(push)
(assert (le x9 java.util.HashMap))
(check-sat)
(pop)
(push)
(assert (le x9 soot.jimple.toolkits.typing.fast.QueuedSet))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x9 java.util.Hashtable)))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x9 java.util.TreeSet))
(check-sat)
(pop)
(push)
(assert (le x9 java.util.AbstractCollection))
(check-sat)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (le x9 java.util.HashSet))
(check-sat)
(pop)
(push)
(assert (le x9 java.util.AbstractMap))
(check-sat)
(push)
(assert (not (le x9 java.util.HashMap)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x9 soot.jimple.spark.ondemand.genericutil.Stack))
(check-sat)
(pop)
(push)
(assert (le x9 soot.jimple.spark.sets.PointsToSetEqualsWrapper))
(check-sat)
(pop)
(push)
(assert (le x9 soot.util.UnitMap))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x9 java.util.Vector))
(check-sat)
(pop)
(push)
(assert (= x9 soot.coffi.BBQ))
(check-sat)
(pop)
(push)
(assert (le x9 java.util.BitSet))
(check-sat)
(pop)
(push)
(assert (le x9 java.util.ArrayList))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(push)
(assert (le x13 soot.MethodSource))
(check-sat)
(pop)
(push)
(assert (not (le x13 soot.MethodSource)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (le x9 soot.jimple.spark.ondemand.genericutil.ArraySet))
(check-sat)
(pop)
(push)
(assert (le x9 soot.util.HashMultiMap))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x2 java.lang.StringBuilder)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x12 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x9 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x3 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x13 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x3 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x14 java.io.File))
(check-sat)
(push)
(assert (= x2 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.StringBuffer)))
(check-sat)
(push)
(assert (le x9 java.lang.Object))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.lang.Object)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x14 java.io.File)))
(check-sat)
(push)
(assert (le x13 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (le x2 java.io.File))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (le x9 soot.util.HashChain_currencysign_Link))
(check-sat)
(push)
(assert (le x3 java.io.File))
(check-sat)
(push)
(assert (le x14 java.util.List))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.util.List)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x3 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x9 soot.util.HashChain_currencysign_Link)))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x9 soot.util.HashChain_currencysign_Link))
(check-sat)
(pop)
(push)
(assert (not (le x9 soot.util.HashChain_currencysign_Link)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(push)
(assert (le x12 java.util.List))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.util.List)))
(check-sat)
(pop)
(push)
(assert (le x12 soot.coffi.CONSTANT_Class_info))
(check-sat)
(push)
(assert (le x15 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x12 soot.coffi.CONSTANT_Class_info)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(push)
(assert (le x8 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(push)
(assert (le x2 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(push)
(assert (le x15 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x15 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x12 soot.coffi.method_info))
(check-sat)
(pop)
(push)
(assert (not (le x12 soot.coffi.method_info)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x4 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x4 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(push)
(assert (le x12 java.io.File))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x15 soot.coffi.CONSTANT_Methodref_info))
(check-sat)
(pop)
(push)
(assert (not (le x15 soot.coffi.CONSTANT_Methodref_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(push)
(assert (le x7 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(push)
(assert (le x6 soot.MethodSource))
(check-sat)
(pop)
(push)
(assert (not (le x6 soot.MethodSource)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(push)
(assert (le x6 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x14 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x14 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x14 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (le x6 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x15 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x8 soot.util.HashChain_currencysign_Link))
(check-sat)
(push)
(assert (le x8 soot.util.HashChain_currencysign_Link))
(check-sat)
(pop)
(push)
(assert (not (le x8 soot.util.HashChain_currencysign_Link)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x8 soot.util.HashChain_currencysign_Link)))
(check-sat)
(push)
(assert (le x6 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.io.File)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x14 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x14 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x14 java.io.DataInputStream))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.io.DataInputStream)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x2 java.io.File))
(check-sat)
(push)
(assert (le x2 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x6 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x6 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(push)
(assert (le x11 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x11 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x8 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x13 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(push)
(assert (le x8 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x8 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x8 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x13 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x4 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x4 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x7 soot.coffi.Code_attribute))
(check-sat)
(pop)
(push)
(assert (not (le x7 soot.coffi.Code_attribute)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x7 java.util.List))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 java.util.List)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (le x6 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(push)
(assert (le x7 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(pop)
(push)
(assert (not (le x7 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x8 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.io.File)))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x13 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x13 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x3 soot.util.HashChain_currencysign_Link))
(check-sat)
(pop)
(push)
(assert (not (le x3 soot.util.HashChain_currencysign_Link)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x3 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.io.File)))
(check-sat)
(push)
(assert (le x11 soot.coffi.CONSTANT_String_info))
(check-sat)
(pop)
(push)
(assert (not (le x11 soot.coffi.CONSTANT_String_info)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x9 java.io.File))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(push)
(assert (le x13 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x6 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x13 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x13 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x10 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x8 java.io.File))
(check-sat)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x8 java.io.File)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x10 java.lang.StringBuilder))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x10 java.lang.StringBuilder))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.StringBuilder)))
(check-sat)
(pop)
(push)
(assert (le x0 java.lang.Object))
(check-sat)
(pop)
(push)
(assert (not (le x0 java.lang.Object)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x10 java.lang.StringBuilder)))
(check-sat)
(push)
(assert (le x0 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x0 java.io.File)))
(check-sat)
(push)
(assert (= x10 java.lang.StringBuilder))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.StringBuilder)))
(check-sat)
(push)
(assert (le x14 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(push)
(assert (le x7 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(push)
(assert (le x8 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x14 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x0 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x0 java.io.File)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (le x2 java.io.File))
(check-sat)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(push)
(assert (le x3 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(push)
(assert (le x2 java.io.File))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x6 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(push)
(assert (le x11 java.util.List))
(check-sat)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x11 java.util.List)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x6 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x12 java.io.File))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(push)
(assert (le x6 java.util.List))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.util.List)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x14 java.lang.Object))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.lang.Object)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x13 soot.Transformer))
(check-sat)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x11 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x13 soot.Transformer)))
(check-sat)
(pop)
(push)
(assert (le x13 java.io.File))
(check-sat)
(push)
(assert (le x11 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x13 java.io.File)))
(check-sat)
(push)
(assert (le x11 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.io.File)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(push)
(assert (le x13 java.io.DataInputStream))
(check-sat)
(push)
(assert (le x12 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x13 java.io.DataInputStream)))
(check-sat)
(push)
(assert (le x10 java.util.List))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.util.List)))
(check-sat)
(pop)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x13 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x10 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x13 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x6 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x8 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x6 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x14 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(pop)
(push)
(assert (not (le x14 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(push)
(assert (le x11 java.util.List))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x11 java.util.List)))
(check-sat)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x6 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(pop)
(push)
(assert (not (le x6 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(push)
(assert (le x11 java.io.File))
(check-sat)
(push)
(assert (le x3 soot.coffi.CONSTANT_Methodref_info))
(check-sat)
(pop)
(push)
(assert (not (le x3 soot.coffi.CONSTANT_Methodref_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x11 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x13 soot.coffi.CONSTANT_Fieldref_info))
(check-sat)
(push)
(assert (le x4 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x4 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x13 soot.coffi.CONSTANT_Fieldref_info)))
(check-sat)
(push)
(assert (le x0 java.io.File))
(check-sat)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x4 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x4 java.io.File)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x0 java.io.File)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (le x2 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(push)
(assert (= x7 java.lang.StringBuilder))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.StringBuilder)))
(check-sat)
(pop)
(push)
(assert (le x12 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (le x7 java.util.List))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.util.List)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x0 soot.coffi.CONSTANT_Fieldref_info))
(check-sat)
(push)
(assert (le x7 java.io.File))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x12 soot.Type))
(check-sat)
(push)
(assert (le x3 java.util.HashMap))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.HashMap)))
(check-sat)
(pop)
(push)
(assert (le x6 java.io.File))
(check-sat)
(push)
(assert (le x6 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x6 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x10 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.io.File)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x12 soot.Type))
(check-sat)
(push)
(assert (le x8 soot.coffi.CONSTANT_Methodref_info))
(check-sat)
(pop)
(push)
(assert (not (le x8 soot.coffi.CONSTANT_Methodref_info)))
(check-sat)
(pop)
(push)
(assert (le x0 soot.coffi.CONSTANT_Fieldref_info))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Fieldref_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x12 soot.Type)))
(check-sat)
(pop)
(push)
(assert (le x7 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x12 soot.Type)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Class_info)))
(check-sat)
(pop)
(push)
(assert (le x0 soot.coffi.CONSTANT_Class_info))
(check-sat)
(pop)
(push)
(assert (le x0 soot.coffi.CONSTANT_Fieldref_info))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Fieldref_info)))
(check-sat)
(pop)
(push)
(assert (le x4 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x4 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(push)
(assert (le x4 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x4 java.io.File)))
(check-sat)
(push)
(assert (le x7 java.io.File))
(check-sat)
(push)
(assert (le x4 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x4 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(push)
(assert (le x12 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x12 java.util.Iterator))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.util.Iterator)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x4 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x4 java.io.File)))
(check-sat)
(push)
(assert (le x14 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x10 java.util.List))
(check-sat)
(push)
(assert (le x13 java.io.File))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x13 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x10 java.util.List)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x3 soot.util.HashChain_currencysign_Link))
(check-sat)
(pop)
(push)
(assert (not (le x3 soot.util.HashChain_currencysign_Link)))
(check-sat)
(pop)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(push)
(assert (le x14 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.io.File)))
(check-sat)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Class_info)))
(check-sat)
(pop)
(push)
(assert (le x0 soot.coffi.CONSTANT_Class_info))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x14 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x10 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.io.File)))
(check-sat)
(push)
(assert (le x13 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x12 java.lang.Object))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (le x14 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (le x12 java.lang.Object)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x0 soot.coffi.CONSTANT_Fieldref_info))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(push)
(assert (le x8 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x10 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.io.File)))
(check-sat)
(push)
(assert (= x1 java.lang.Integer))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.Integer)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x11 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.StringBuilder))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.StringBuilder)))
(check-sat)
(pop)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x11 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(push)
(assert (le x11 java.io.DataInputStream))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.io.DataInputStream)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Fieldref_info)))
(check-sat)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x8 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(push)
(assert (le x10 java.lang.Object))
(check-sat)
(push)
(assert (le x13 soot.coffi.Exception_attribute))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x13 soot.coffi.Exception_attribute)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x10 java.lang.Object)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(push)
(assert (le x15 java.io.DataInputStream))
(check-sat)
(push)
(assert (le x14 java.lang.Object))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.lang.Object)))
(check-sat)
(push)
(assert (not (le x14 soot.RefType)))
(check-sat)
(pop)
(push)
(assert (le x14 soot.RefType))
(check-sat)
(pop)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (le x15 java.io.DataInputStream)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x0 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(push)
(assert (= x11 java.lang.StringBuilder))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x14 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x11 java.lang.StringBuilder)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (le x3 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x13 java.io.DataInputStream))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.io.DataInputStream)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x8 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x7 soot.coffi.CONSTANT_Methodref_info))
(check-sat)
(push)
(assert (le x13 java.io.File))
(check-sat)
(push)
(assert (le x2 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x13 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 soot.coffi.CONSTANT_Methodref_info)))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x8 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (le x8 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(pop)
(push)
(assert (not (le x8 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(push)
(assert (le x8 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(pop)
(push)
(assert (not (le x8 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x12 soot.util.HashChain_currencysign_Link))
(check-sat)
(push)
(assert (le x2 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x12 soot.util.HashChain_currencysign_Link)))
(check-sat)
(push)
(assert (le x13 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x12 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(push)
(assert (le x0 soot.coffi.CONSTANT_NameAndType_info))
(check-sat)
(push)
(assert (le x8 java.lang.Object))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x7 java.io.File))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x8 java.lang.Object)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_NameAndType_info)))
(check-sat)
(push)
(assert (= x15 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.StringBuffer)))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (le x4 java.util.List))
(check-sat)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Class_info)))
(check-sat)
(pop)
(push)
(assert (le x0 soot.coffi.CONSTANT_Class_info))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x4 java.util.List)))
(check-sat)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x13 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(push)
(assert (le x2 java.io.DataInputStream))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.io.DataInputStream)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(push)
(assert (le x0 soot.coffi.CONSTANT_Fieldref_info))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Fieldref_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x4 java.util.List))
(check-sat)
(pop)
(push)
(assert (not (le x4 java.util.List)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x2 java.io.File))
(check-sat)
(push)
(assert (le x10 soot.coffi.CONSTANT_Methodref_info))
(check-sat)
(pop)
(push)
(assert (not (le x10 soot.coffi.CONSTANT_Methodref_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x6 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (le x3 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x0 soot.coffi.CONSTANT_Fieldref_info))
(check-sat)
(pop)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Fieldref_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (le x12 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x0 soot.coffi.CONSTANT_Fieldref_info))
(check-sat)
(push)
(assert (le x10 java.io.DataInputStream))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.io.DataInputStream)))
(check-sat)
(push)
(assert (le x0 soot.coffi.CONSTANT_Fieldref_info))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Fieldref_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x15 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x15 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x11 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x7 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Fieldref_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(push)
(assert (le x1 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x2 java.io.File))
(check-sat)
(push)
(assert (le x15 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(push)
(assert (le x2 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x3 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x2 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x11 java.io.File))
(check-sat)
(push)
(assert (le x1 soot.MethodSource))
(check-sat)
(pop)
(push)
(assert (not (le x1 soot.MethodSource)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x11 java.io.File)))
(check-sat)
(push)
(assert (le x7 soot.MethodSource))
(check-sat)
(pop)
(push)
(assert (not (le x7 soot.MethodSource)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (le x11 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x14 java.io.InputStream))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(pop)
(push)
(assert (le x0 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x14 java.io.InputStream)))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x2 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x2 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(push)
(assert (le x2 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(pop)
(push)
(assert (not (le x2 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Class_info)))
(check-sat)
(pop)
(push)
(assert (le x0 soot.coffi.CONSTANT_Class_info))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(push)
(assert (= x13 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (le x0 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(pop)
(push)
(assert (le x3 java.util.List))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.List)))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Fieldref_info)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x0 java.lang.StringBuilder))
(check-sat)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x0 java.lang.StringBuilder)))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (le x15 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(push)
(assert (le x15 java.util.List))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x12 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x12 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x15 java.util.List)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x8 soot.CoffiClassSource))
(check-sat)
(pop)
(push)
(assert (not (le x8 soot.CoffiClassSource)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (le x0 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x0 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(push)
(assert (le x12 java.io.File))
(check-sat)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(push)
(assert (le x11 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(push)
(assert (le x1 java.io.File))
(check-sat)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(push)
(assert (le x7 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (le x4 soot.coffi.Code_attribute))
(check-sat)
(pop)
(push)
(assert (not (le x4 soot.coffi.Code_attribute)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x1 java.io.File)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (le x1 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (le x10 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.io.File)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (= x8 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x4 soot.coffi.Code_attribute))
(check-sat)
(pop)
(push)
(assert (not (le x4 soot.coffi.Code_attribute)))
(check-sat)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(push)
(assert (le x11 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.io.File)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x8 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x12 soot.MethodSource))
(check-sat)
(pop)
(push)
(assert (not (le x12 soot.MethodSource)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (le x2 soot.util.HashChain_currencysign_Link))
(check-sat)
(pop)
(push)
(assert (not (le x2 soot.util.HashChain_currencysign_Link)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (le x6 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.io.File)))
(check-sat)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.StringBuffer)))
(check-sat)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(push)
(assert (le x1 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.io.File)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (le x10 soot.util.HashChain_currencysign_Link))
(check-sat)
(pop)
(push)
(assert (not (le x10 soot.util.HashChain_currencysign_Link)))
(check-sat)
(pop)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x6 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x11 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(push)
(assert (le x0 soot.coffi.method_info))
(check-sat)
(pop)
(push)
(assert (not (le x0 soot.coffi.method_info)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x0 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x0 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x14 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(push)
(assert (le x6 soot.coffi.Code_attribute))
(check-sat)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(push)
(assert (le x11 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(pop)
(push)
(assert (not (le x11 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(push)
(assert (le x7 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x6 soot.coffi.Code_attribute))
(check-sat)
(pop)
(push)
(assert (not (le x6 soot.coffi.Code_attribute)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x11 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x10 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x12 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x12 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x1 java.io.File))
(check-sat)
(push)
(assert (le x1 java.io.File))
(check-sat)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x8 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(pop)
(push)
(assert (not (le x8 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x0 java.util.List))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x0 java.util.List)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x1 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x1 java.io.File)))
(check-sat)
(push)
(assert (le x12 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(push)
(assert (le x2 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(pop)
(push)
(assert (not (le x2 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x8 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x1 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x12 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x8 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.io.File)))
(check-sat)
(push)
(assert (le x14 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(pop)
(push)
(assert (not (le x14 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (le x6 soot.coffi.Code_attribute)))
(check-sat)
(pop)
(push)
(assert (le x8 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x6 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x6 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(push)
(assert (le x1 soot.coffi.Code_attribute))
(check-sat)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(push)
(assert (le x14 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(pop)
(push)
(assert (not (le x14 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x1 soot.coffi.Code_attribute)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(push)
(assert (le x10 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x10 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x0 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x0 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x9 java.lang.Object))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.lang.Object)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (le x9 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x1 soot.coffi.CONSTANT_Fieldref_info))
(check-sat)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x1 soot.coffi.CONSTANT_Fieldref_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x7 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(pop)
(push)
(assert (not (le x7 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x11 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x11 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(push)
(assert (le x2 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (le x12 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x11 java.io.File))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x11 java.io.File)))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x14 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(push)
(assert (le x7 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(pop)
(push)
(assert (not (le x7 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(push)
(assert (le x9 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x14 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x0 java.lang.Object))
(check-sat)
(pop)
(push)
(assert (not (le x0 java.lang.Object)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(push)
(assert (= x12 java.lang.StringBuilder))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(push)
(assert (le x3 soot.coffi.CONSTANT_Utf8_info))
(check-sat)
(pop)
(push)
(assert (not (le x3 soot.coffi.CONSTANT_Utf8_info)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x12 java.lang.StringBuilder)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x13 soot.util.HashChain_currencysign_Link))
(check-sat)
(pop)
(push)
(assert (not (le x13 soot.util.HashChain_currencysign_Link)))
(check-sat)
(push)
(assert (le x6 java.io.DataInputStream))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.io.DataInputStream)))
(check-sat)
(push)
(assert (le x15 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(push)
(assert (le x7 soot.coffi.method_info))
(check-sat)
(pop)
(push)
(assert (not (le x7 soot.coffi.method_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x14 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(pop)
(push)
(assert (not (le x14 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(pop)
(push)
(assert (= x12 java.lang.StringBuilder))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (= x3 java.lang.StringBuilder))
(check-sat)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x3 java.lang.StringBuilder)))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x2 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(push)
(assert (= x3 java.lang.StringBuilder))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.StringBuilder)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x10 java.io.File))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x11 java.lang.StringBuilder))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.StringBuilder)))
(check-sat)
(pop)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x11 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x10 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x11 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x12 java.lang.StringBuilder))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x12 java.lang.StringBuilder)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x11 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x9 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (le x14 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(push)
(assert (= x11 java.lang.StringBuilder))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.StringBuilder)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x14 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (le x11 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.io.File)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (le x0 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x0 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x12 java.lang.StringBuilder)))
(check-sat)
(push)
(assert (le x15 soot.coffi.CONSTANT_NameAndType_info))
(check-sat)
(pop)
(push)
(assert (not (le x15 soot.coffi.CONSTANT_NameAndType_info)))
(check-sat)
(pop)
(push)
(assert (le x14 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(pop)
(push)
(assert (not (le x14 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x14 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x1 soot.coffi.CONSTANT_Methodref_info))
(check-sat)
(pop)
(push)
(assert (not (le x1 soot.coffi.CONSTANT_Methodref_info)))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x0 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x0 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x8 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x1 java.util.List))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.util.List)))
(check-sat)
(pop)
(push)
(assert (le x11 java.lang.Object))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(push)
(assert (le x15 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x14 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(push)
(assert (le x15 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x14 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(push)
(assert (le x0 soot.coffi.CONSTANT_Methodref_info))
(check-sat)
(pop)
(push)
(assert (not (le x0 soot.coffi.CONSTANT_Methodref_info)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x6 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(pop)
(push)
(assert (not (le x6 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x10 java.util.List))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.util.List)))
(check-sat)
(pop)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(push)
(assert (le x12 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (le x11 java.lang.Object)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x14 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(push)
(assert (le x2 java.io.DataInputStream))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (le x8 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x8 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x2 java.io.DataInputStream)))
(check-sat)
(pop)
(push)
(assert (= x7 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.StringBuffer)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x14 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x0 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x0 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x14 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x0 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x0 java.io.File)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (le x8 java.util.HashSet))
(check-sat)
(push)
(assert (le x14 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(push)
(assert (le x8 java.util.HashSet))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.util.HashSet)))
(check-sat)
(pop)
(push)
(assert (= x9 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x7 java.io.File))
(check-sat)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x14 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(pop)
(push)
(assert (le x7 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x8 java.util.HashSet))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.util.HashSet)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x8 java.util.HashSet)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x14 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(pop)
(push)
(assert (not (le x14 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (le x8 java.util.List))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.util.List)))
(check-sat)
(push)
(assert (le x11 java.io.File))
(check-sat)
(push)
(assert (= x2 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x7 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x7 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x2 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x11 java.io.File)))
(check-sat)
(push)
(assert (le x11 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (le x9 soot.MethodSource))
(check-sat)
(pop)
(push)
(assert (not (le x9 soot.MethodSource)))
(check-sat)
(push)
(assert (= x10 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.StringBuffer)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x11 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x9 soot.MethodSource))
(check-sat)
(pop)
(push)
(assert (not (le x9 soot.MethodSource)))
(check-sat)
(pop)
(push)
(assert (le x2 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x11 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(push)
(assert (le x12 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x0 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x0 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x14 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(push)
(assert (le x10 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x2 java.io.DataInputStream))
(check-sat)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(push)
(assert (le x2 java.io.DataInputStream))
(check-sat)
(pop)
(push)
(assert (not (le x2 java.io.DataInputStream)))
(check-sat)
(pop)
(push)
(assert (not (le x12 soot.shimple.internal.SPatchingChain)))
(check-sat)
(push)
(assert (le x12 soot.PatchingChain))
(check-sat)
(pop)
(pop)
(push)
(assert (le x12 java.util.LinkedList))
(check-sat)
(push)
(assert (not (le x12 soot.jimple.parser.node.TypedLinkedList)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x12 soot.shimple.internal.SPatchingChain))
(check-sat)
(pop)
(push)
(assert (le x12 soot.jimple.parser.node.TypedLinkedList))
(check-sat)
(pop)
(push)
(assert (le x12 soot.util.HashChain))
(check-sat)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x2 java.io.DataInputStream)))
(check-sat)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (le x12 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(push)
(assert (le x8 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x8 java.io.File))
(check-sat)
(push)
(assert (le x12 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(push)
(assert (le x15 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x13 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x13 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x2 java.io.File))
(check-sat)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x8 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(push)
(assert (le x12 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x6 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.io.File)))
(check-sat)
(push)
(assert (le x7 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (le x7 soot.util.HashChain_currencysign_Link))
(check-sat)
(pop)
(push)
(assert (not (le x7 soot.util.HashChain_currencysign_Link)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (le x10 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x6 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x6 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x8 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(push)
(assert (= x14 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x14 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(push)
(assert (le x11 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(push)
(assert (le x8 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(push)
(assert (= x11 java.lang.String))
(check-sat)
(push)
(assert (le x15 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x15 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x11 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (le x6 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x6 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(push)
(assert (le x8 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x8 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x9 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (= x2 java.lang.String))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x2 java.lang.String)))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(push)
(assert (le x11 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x9 java.io.File))
(check-sat)
(push)
(assert (le x1 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x1 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x10 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x10 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (le x3 soot.util.HashChain_currencysign_Link))
(check-sat)
(push)
(assert (= x4 java.lang.String))
(check-sat)
(push)
(assert (le x15 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x12 soot.coffi.CONSTANT_Methodref_info))
(check-sat)
(pop)
(push)
(assert (not (le x12 soot.coffi.CONSTANT_Methodref_info)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x15 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x13 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x3 soot.util.HashChain_currencysign_Link)))
(check-sat)
(pop)
(push)
(assert (= x0 java.lang.String))
(check-sat)
(push)
(assert (= x3 java.lang.String))
(check-sat)
(push)
(assert (le x8 soot.util.HashChain_currencysign_Link))
(check-sat)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x8 soot.util.HashChain_currencysign_Link)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x3 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x13 java.io.File))
(check-sat)
(push)
(assert (le x3 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x10 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.util.zip.ZipEntry)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x13 java.io.File)))
(check-sat)
(push)
(assert (le x13 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.io.File)))
(check-sat)
(push)
(assert (le x9 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.io.File)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(pop)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (= x12 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x12 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x11 java.lang.Object))
(check-sat)
(pop)
(push)
(assert (not (le x11 java.lang.Object)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(push)
(assert (le x13 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.io.File)))
(check-sat)
(push)
(assert (= x9 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x9 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x12 java.io.File))
(check-sat)
(push)
(assert (le x13 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x7 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x7 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x9 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x9 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x1 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x1 java.lang.String)))
(check-sat)
(push)
(assert (le x3 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x3 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x10 soot.SourceLocator_currencysign_FoundFile))
(check-sat)
(pop)
(push)
(assert (not (le x10 soot.SourceLocator_currencysign_FoundFile)))
(check-sat)
(pop)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x13 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(push)
(assert (le x13 java.io.File))
(check-sat)
(push)
(assert (le x13 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x13 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x13 java.io.File)))
(check-sat)
(push)
(assert (= x15 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x15 java.lang.String)))
(check-sat)
(push)
(assert (le x8 java.util.zip.ZipEntry))
(check-sat)
(push)
(assert (le x12 java.io.File))
(check-sat)
(push)
(assert (le x12 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x12 java.io.File)))
(check-sat)
(push)
(assert (le x2 java.io.File))
(check-sat)
(push)
(assert (= x5 java.lang.StringBuffer))
(check-sat)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (le x2 java.io.File)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (le x8 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (= x8 java.lang.String))
(check-sat)
(pop)
(push)
(assert (not (= x8 java.lang.String)))
(check-sat)
(pop)
(push)
(assert (le x14 java.util.zip.ZipEntry))
(check-sat)
(pop)
(push)
(assert (not (le x14 java.util.zip.ZipEntry)))
(check-sat)
(pop)
(push)
(assert (le x10 java.io.File))
(check-sat)
(pop)
(push)
(assert (not (le x10 java.io.File)))
(check-sat)
(pop)
(pop)
(push)
(assert (le x10 soot.coffi.Exception_attribute))
(check-sat)
(pop)
(push)
(assert (not (le x10 soot.coffi.Exception_attribute)))
(check-sat)
(pop)
(pop)
(pop)
(push)
(assert (not (= x0 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x4 java.lang.String)))
(check-sat)
(pop)
(pop)
(push)
(assert (not (= x5 java.lang.StringBuffer)))
(check-sat)
(pop)
